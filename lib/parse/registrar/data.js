...
var getMethodName = function getMethodName(type) {
  return camelCase.apply(undefined, _toConsumableArray(type.match(_type.typeMatcher).slice(1).filter(Boolean)));
};
...
var addDataParser = function addDataParser(format, parser) {
  ...
  var methodName = methodNames[format] = methodNames[format] || getMethodName(format);
